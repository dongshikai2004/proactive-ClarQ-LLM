...
class player:
    ...
    def data2prompt(self, content):
        ...
        if detect_language(content[0]) == "Chinese":
            ...
            addition= "Talk directly with Jax. The word 'Goodbye' cannot appear in the conversation before it ends. Only the conversation content, do not reply with other irrelevant content, and do not include extra parts like Reason.Don't imagine yourself as Jax; only respond to what you would say to Jax."
            return start + obj_skill_scenarios + end + addition
